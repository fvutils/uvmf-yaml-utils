...
#      'qvip_library':QVIPLibValidator(),   ## Don't validate QVIP library info, for debug purposes only
  ...
  def buildElements(self,genarray,verify=True,gencode=True):
    count = 0
    self.interfaceDict = {}
    try:
      arrlen = len(genarray)
    ...
    for interface_name in self.data['interfaces']:
      if ((arrlen>0) and (interface_name in genarray)) or (arrlen==0):
        self.interfaceDict[interface_name] = self.generateInterface(interface_name,gencode=gencode)
        count = count + 1 
    ...
    for environment_name in self.data['environments']:
      if ((arrlen>0) and (environment_name in genarray)) or (arrlen==0):
        self.environmentDict[environment_name] = self.generateEnvironment(environment_name,gencode=gencode)
        count = count + 1 
    self.benchDict = {}
    for bench_name in self.data['benches']:
      if ((arrlen>0) and (bench_name in genarray)) or (arrlen==0):
        self.benchDict[bench_name] = self.generateBench(bench_name,gencode=gencode)
        count = count + 1
  ...
  def generateEnvironment(self,name,gencode=True):
  ...
  def generateBench(self,name,gencode=True):
  ...
  def generateInterface(self,name,gencode=True):
